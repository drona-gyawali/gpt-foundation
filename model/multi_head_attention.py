...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        for _ in range(num_heads):
            temp_list.append(self.SingleHeadAttention(embedding_dim, self.head_size))
        self.head = nn.ModuleList(temp_list)
        self.output_projections = nn.Linear(attention_dim, attention_dim, bias= False)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        output = self.output_projections(concat)
